...
export class Select<T> {
   ...
   *
   */
  private falsifyChedkedArray(): void {
    this.checkedArray = new Array(this.mmatrix.getDx());
    this.checkedArray.fill(new Array<boolean>(this.mmatrix.getDy()));
  }

  /**
   * Checks if provided data validate condition given by dx and dy.
   *
   * @param {(a:T,b:T)=>boolean} function defining order between two T elements
   * @returns {void} `
   *
   */
   ...
   *
   */
  private addFromNeighbourhood(
    x: number,
    y: number,
    neighbourhood: SSet<[number, number]>,
    neighbourhoods: Set<SSet<[number, number]>>
  ): boolean {
    ...
    [[0, 1], [0, -1], [1, 0], [1, 0]].forEach(c => {
      let i = c[0],
        j = c[1];
      if (
        !this.mmatrix.validateX(x + i) ||
        !this.mmatrix.validateY(y + j) ||
        this.checkedArray[x + i][y + j]
      ) {
        return;
      }

      this.array[x][y] !== this.array[x + i][y + j] ||
        !neighbourhood.sadd([x + i, y + j]) ||
  ...
  public findCoulpes(): Set<SSet<[number, number]>> {
    ...
    for (let i: number = 0, max: number = this.array.length; i < max; i++) {
      for (
        let j: number = 0, max0: number = this.array[0].length;
        j < max0;
        j++
      ) {
        this.checkedArray[i][j] ||
          !(this.checkedArray[i][j] = true) ||
          !currentNeighbourhoodArr.push(new SSet<[number, number]>()) ||
          this.addFromNeighbourhood(
            i,
            j,
            currentNeighbourhoodArr[currentNeighbourhoodArr.length - 1],
            neighbourhoods
          );
      }
    }
